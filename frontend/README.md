...
результату можно скачать PDF-отчёт и заполненный расчётный шаблон MONSters в формате
XLSX. Excel-файл каждый раз формируется из чистого шаблона по результату выбранного
запуска анализа.
