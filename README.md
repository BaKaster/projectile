...
Production-книга формируется как копия утверждённого `Шаблон.xlsx`: сервис находит
входные столбцы по заголовкам нового шаблона, сохраняет формулы и, если настроен
`PROJECTILE_EXCEL_RECALCULATION_COMMAND`, пересчитывает и сохраняет книгу серверным
табличным движком до возврата клиенту.
...
разворачивает назначения ролей в строки расчёта, записывает в столбец G обоснование часов
для каждой роли, переносит клиентское объяснение этапов на итоговый лист, а также сохраняет
существенные риски и допущения. Каждый запрос начинает работу с чистого шаблона, поэтому
данные разных проектов и повторных анализов не смешиваются.
...

{
      ...
      "role_assignments": [
        {
          ...
          "estimated_hours": 24,
          "hours_rationale": "Три рабочих дня на разработку и проверку архитектуры"
        },
  ...
  "stage_explanations": [
    {
      "stage_no": 1,
      "explanation": "Нужен для согласования целевой архитектуры до внедрения"
    }
  ],
  "external_costs": [],
  "assumptions": []
...
одну роль. Лимиты проверяются после разворачивания: 50 строк работ, 20 внешних затрат и
